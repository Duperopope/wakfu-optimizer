#!/usr/bin/env python3
"""
sync_wakfuli.py — Extraction complète de l'API Wakfuli
======================================================
Phase 1 du pipeline Wakfu Optimizer.
Source : https://api.wakfuli.com/api/v1/
Version Ankama : récupérée dynamiquement depuis wakfu.cdn.ankama.com/gamedata/config.json

Ce script :
  1. Télécharge les 7 261+ items (paginés par 100)
  2. Télécharge les 68 actions/effets (une seule page)
  3. Télécharge les builds publics (paginés par 50)
  4. Sauvegarde tout dans data/wakfuli/ avec métadonnées de version
  5. Génère un rapport de synchronisation dans logs/
...
def sync_builds():
    ...
    for build in builds:
        classes_found.add(build.get("characterClass", "unknown"))
