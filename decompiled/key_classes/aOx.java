/*
 * Decompiled with CFR 0.152.
 * 
 * Could not load the following classes:
 *  aOA
 *  aOy
 *  aOz
 *  aqH
 *  aqz
 *  ewj
 */
import java.util.HashMap;

public class aOx
implements aqz {
    protected int o;
    protected int egP;
    protected int ciZ;
    protected short aWK;
    protected short aZk;
    protected short erz;
    protected float erA;
    protected float erB;
    protected short erC;
    protected boolean erD;
    protected boolean erE;
    protected boolean erF;
    protected boolean erG;
    protected boolean erH;
    protected boolean erI;
    protected boolean erJ;
    protected int erK;
    protected String erL;
    protected short erM;
    protected float erN;
    protected float erO;
    protected float erP;
    protected float erQ;
    protected float erR;
    protected float erS;
    protected float erT;
    protected float erU;
    protected float erV;
    protected float erW;
    protected short erX;
    protected short erY;
    protected short erZ;
    protected short esa;
    protected short esb;
    protected String eqF;
    protected String esc;
    protected byte esd;
    protected boolean ese;
    protected boolean esf;
    protected boolean esg;
    protected byte esh;
    protected boolean esi;
    protected boolean esj;
    protected boolean esk;
    protected boolean esl;
    protected boolean esm;
    protected boolean esn;
    protected int[] baD;
    protected int[] egL;
    protected HashMap<Byte, aOz> eso;
    protected HashMap<Short, aOA> esp;
    protected HashMap<String, aOy> esq;

    public int d() {
        return this.o;
    }

    public int cbt() {
        return this.egP;
    }

    public int aVt() {
        return this.ciZ;
    }

    public short aVe() {
        return this.aWK;
    }

    public short aWP() {
        return this.aZk;
    }

    public short cvb() {
        return this.erz;
    }

    public float cvc() {
        return this.erA;
    }

    public float cvd() {
        return this.erB;
    }

    public short cve() {
        return this.erC;
    }

    public boolean cvf() {
        return this.erD;
    }

    public boolean cvg() {
        return this.erE;
    }

    public boolean cvh() {
        return this.erF;
    }

    public boolean cvi() {
        return this.erG;
    }

    public boolean cvj() {
        return this.erH;
    }

    public boolean cvk() {
        return this.erI;
    }

    public boolean cvl() {
        return this.erJ;
    }

    public int cvm() {
        return this.erK;
    }

    public String cvn() {
        return this.erL;
    }

    public short cvo() {
        return this.erM;
    }

    public float cvp() {
        return this.erN;
    }

    public float cvq() {
        return this.erO;
    }

    public float cvr() {
        return this.erP;
    }

    public float cvs() {
        return this.erQ;
    }

    public float cvt() {
        return this.erR;
    }

    public float cvu() {
        return this.erS;
    }

    public float cvv() {
        return this.erT;
    }

    public float cvw() {
        return this.erU;
    }

    public float cvx() {
        return this.erV;
    }

    public float cvy() {
        return this.erW;
    }

    public short cvz() {
        return this.erX;
    }

    public short cvA() {
        return this.erY;
    }

    public short cvB() {
        return this.erZ;
    }

    public short cvC() {
        return this.esa;
    }

    public short cvD() {
        return this.esb;
    }

    public String cuh() {
        return this.eqF;
    }

    public String cvE() {
        return this.esc;
    }

    public byte cvF() {
        return this.esd;
    }

    public boolean cvG() {
        return this.ese;
    }

    public boolean cvH() {
        return this.esf;
    }

    public boolean cvI() {
        return this.esg;
    }

    public byte cvJ() {
        return this.esh;
    }

    public boolean cvK() {
        return this.esi;
    }

    public boolean cvL() {
        return this.esj;
    }

    public boolean cvM() {
        return this.esk;
    }

    public boolean cvN() {
        return this.esl;
    }

    public boolean cvO() {
        return this.esm;
    }

    public boolean cvP() {
        return this.esn;
    }

    public int[] ckm() {
        return this.baD;
    }

    public int[] cjX() {
        return this.egL;
    }

    public HashMap<Byte, aOz> cvQ() {
        return this.eso;
    }

    public HashMap<Short, aOA> cvR() {
        return this.esp;
    }

    public HashMap<String, aOy> cvS() {
        return this.esq;
    }

    public void reset() {
        this.o = 0;
        this.egP = 0;
        this.ciZ = 0;
        this.aWK = 0;
        this.aZk = 0;
        this.erz = 0;
        this.erA = 0.0f;
        this.erB = 0.0f;
        this.erC = 0;
        this.erD = false;
        this.erE = false;
        this.erF = false;
        this.erG = false;
        this.erH = false;
        this.erI = false;
        this.erJ = false;
        this.erK = 0;
        this.erL = null;
        this.erM = 0;
        this.erN = 0.0f;
        this.erO = 0.0f;
        this.erP = 0.0f;
        this.erQ = 0.0f;
        this.erR = 0.0f;
        this.erS = 0.0f;
        this.erT = 0.0f;
        this.erU = 0.0f;
        this.erV = 0.0f;
        this.erW = 0.0f;
        this.erX = 0;
        this.erY = 0;
        this.erZ = 0;
        this.esa = 0;
        this.esb = 0;
        this.eqF = null;
        this.esc = null;
        this.esd = 0;
        this.ese = false;
        this.esf = false;
        this.esg = false;
        this.esh = 0;
        this.esi = false;
        this.esj = false;
        this.esk = false;
        this.esl = false;
        this.esm = false;
        this.esn = false;
        this.baD = null;
        this.egL = null;
        this.eso = null;
        this.esp = null;
        this.esq = null;
    }

    public void a(aqH aqH2) {
        int n;
        int n2;
        this.o = aqH2.bGI();
        this.egP = aqH2.bGI();
        this.ciZ = aqH2.bGI();
        this.aWK = aqH2.bGG();
        this.aZk = aqH2.bGG();
        this.erz = aqH2.bGG();
        this.erA = aqH2.bGH();
        this.erB = aqH2.bGH();
        this.erC = aqH2.bGG();
        this.erD = aqH2.bxv();
        this.erE = aqH2.bxv();
        this.erF = aqH2.bxv();
        this.erG = aqH2.bxv();
        this.erH = aqH2.bxv();
        this.erI = aqH2.bxv();
        this.erJ = aqH2.bxv();
        this.erK = aqH2.bGI();
        this.erL = aqH2.bGL().intern();
        this.erM = aqH2.bGG();
        this.erN = aqH2.bGH();
        this.erO = aqH2.bGH();
        this.erP = aqH2.bGH();
        this.erQ = aqH2.bGH();
        this.erR = aqH2.bGH();
        this.erS = aqH2.bGH();
        this.erT = aqH2.bGH();
        this.erU = aqH2.bGH();
        this.erV = aqH2.bGH();
        this.erW = aqH2.bGH();
        this.erX = aqH2.bGG();
        this.erY = aqH2.bGG();
        this.erZ = aqH2.bGG();
        this.esa = aqH2.bGG();
        this.esb = aqH2.bGG();
        this.eqF = aqH2.bGL().intern();
        this.esc = aqH2.bGL().intern();
        this.esd = aqH2.aTf();
        this.ese = aqH2.bxv();
        this.esf = aqH2.bxv();
        this.esg = aqH2.bxv();
        this.esh = aqH2.aTf();
        this.esi = aqH2.bxv();
        this.esj = aqH2.bxv();
        this.esk = aqH2.bxv();
        this.esl = aqH2.bxv();
        this.esm = aqH2.bxv();
        this.esn = aqH2.bxv();
        this.baD = aqH2.bGM();
        this.egL = aqH2.bGM();
        int n3 = aqH2.bGI();
        this.eso = new HashMap(n3);
        for (n2 = 0; n2 < n3; ++n2) {
            byte by = aqH2.aTf();
            aOz aOz2 = new aOz();
            aOz2.a(aqH2);
            this.eso.put(by, aOz2);
        }
        n2 = aqH2.bGI();
        this.esp = new HashMap(n2);
        for (n = 0; n < n2; ++n) {
            short s = aqH2.bGG();
            aOA aOA2 = new aOA();
            aOA2.a(aqH2);
            this.esp.put(s, aOA2);
        }
        n = aqH2.bGI();
        this.esq = new HashMap(n);
        for (int i = 0; i < n; ++i) {
            String string = aqH2.bGL().intern();
            aOy aOy2 = new aOy();
            aOy2.a(aqH2);
            this.esq.put(string, aOy2);
        }
    }

    public final int bGA() {
        return ewj.ozD.d();
    }
}
